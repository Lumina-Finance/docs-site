...
:::danger[Enrolling a passkey for the first time]
Please make sure you have another MFA method before trying to enrol a passkey for the first time to avoid being locked out due to misconfigurations.
:::

## Setting the URL

Set [`APP_URL`](environment-variables.md#app_url) to an address that you will use to access Lumina Finance. For example, `https://lumina-finance.example.com`, **without the trailing slash at the end**. Include the scheme and any non-default port, but not a path such as `/api`.

LF uses this address for password-reset links and sign-in redirects, and includes it in its allowed browser origins (CORS). We recommend setting `APP_URL` explicitly, since leaving it empty will default to allowing all origins. `WEBAUTHN_RP_ID` and `WEBAUTHN_ORIGINS` will be derived from `APP_URL` unless you set them explicitly, and you do not need to make any further adjustments if you access Lumina Finance at the address set in `APP_URL`.
